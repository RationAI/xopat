                        ...
                        const docHandler = (ev) => {
                            try {
                                ...
                                openWrappers.forEach((wrapper) => {
                                    // if click is on the header button for this tab, keep it (the button's own handler will toggle)
                                    const btnEl = document.getElementById(btnId);
                                    if (btnEl && (btnEl === ev.target || btnEl.contains(ev.target))) return;
                                    // otherwise collapse (this makes any click — including clicks on items — close the dropdown)
                                    try { wrapper.classList.remove('dropdown-open'); } catch(_) {}
                                });
                                // also hide the floating recent panel if present
                                try { tab.hideRecent?.(); } catch(_) {}
                            } catch (_) { /* swallow */ }
                        };
                        // use capture so we run before other handlers that may open dropdowns
                        document.addEventListener('click', docHandler, true);
                        // also close on Escape key
                        const keyHandler = (ev) => { if (ev.key === 'Escape') { try { Array.from(document.querySelectorAll('.dropdown.dropdown-open')).forEach(w=>w.classList.remove('dropdown-open')); try { tab.hideRecent?.(); } catch(_){} } catch(_){} } };
