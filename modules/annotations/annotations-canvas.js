OSDAnnotations.FabricWrapper = class OSDAnnotationsFabricWrapper extends XOpatViewerSingleton {
     ...
     *
     * The idea behind fabric exporting is to use module._exportedProps to ensure all properties
    ...
    toObject(withAllProps=true, filter=false, ...withProperties) {
        ...
        if (typeof withAllProps === "boolean") {
            props = this.module._exportedProps(withAllProps);
        } else if (typeof withAllProps === "string") {
            props = this.module._exportedProps(true);
...
// backward-compatible aliases
    ...
    _setListeners() {
        ...
        function handleLeftClickDown(event, fabricEvent) {
            ...
            _this.module.cursor.mouseTime = Date.now();
            // Auto-select a preset on click-down when none is bound to the left
            // button, so the mode receives a factory instead of aborting into the
            // "no preset" warning dialog. Prefers an existing preset; only
            // fabricates a default one when the deployment opts in.
            let preset = _this.module.presets.ensureActivePreset(true);
        ...
        this.canvas.on('mouse:move', function (o) {
            if (_this.module.disabledInteraction) return;
            if (_this.module.cursor.isDown) {
                // A draw started in another viewport owns the shared mode + cursor.
                // This wrapper is only a viewport the pointer wandered into; its
                // screenToPixelCoords are in a different slide's coordinate space, so
                // feeding them would corrupt the in-progress annotation. Freeze until
                // the pointer returns to the drawing viewport (module.viewer stays
                // pinned to it via locksViewer while cursor.isDown).
                if (_this.viewer !== _this.module.viewer) return;
