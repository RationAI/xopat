 ...
 * chose.
 *
 * @module webtiff/auto-config
 */

/** Marks a shader config this module derived rather than a human authored. */
export const AUTO_TAG = "webtiff@1";

/**
 * Configurations the predecessor module derived, recognised so a session saved
 * while `geotiff` was the installed decoder is re-derived here instead of being
 * treated as a user's own choice and left alone.
 */
const LEGACY_TAGS = ["geotiff@1"];

const isAutoTag = (value) => value === AUTO_TAG || LEGACY_TAGS.includes(value);

/**
 * Below this the slide cannot reach half intensity and is, for practical
 * purposes, a black frame — the only case an automatic window is worth its
 ...
 * Same reasoning as {@link shaderSupportsWindow}.
 * @return {boolean}
 */
...
export function shadersAreAutoOwned(background) {
    ...
    return shaders.every(entry => entry && isAutoTag(entry.autoDerived));
}
 ...
 * persisting them into a session only risks carrying a stale layout — including
 * one derived by the `geotiff` module this one replaces.
 *
...
export function stripAutoDerived(backgrounds) {
    ...
    for (const background of backgrounds) {
        if (background && Array.isArray(background.shaders) && background.shaders.length
            && background.shaders.every(entry => entry && isAutoTag(entry.autoDerived))) {
