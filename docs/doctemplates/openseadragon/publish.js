...
if (osdConfig.useHighlightJs) {
    ...
    // Register the JavaScript language with hljs
...
if (osdConfig.debugMode) {
    ...
    debugHtml = '<h1>xOpat Documentation Build</h1>\n<h3>JSDOC Configuration</h3>\n<pre class="source-code">\nenv.conf =\n' + JSON.stringify(env.conf, null, 2) + ';\n</pre>\n';
    debugHtmlHeader = '\
        <!DOCTYPE html>\n\
        <html lang="en">\n\
        <head>\n\
            <meta charset="utf-8">\n\
            <title>xOpat Documentation Build Debug</title>\n\
            \n\
            ...
            <link type="text/css" rel="stylesheet" href="styles/primer_css.css">\n\
        </head>\n\
        \n\
        <body>\n\
...
function generateSourceLineNumberIds(source) {
    ...
    numbered = numbered.map(function(item) {
        counter++;
        return '<span id="line' + counter + '" data-line="'+counter+'" class="line"></span>' + item;
    });
...
function generateSourceFiles(sourceFiles, encoding) {
    ...
    Object.keys(sourceFiles).forEach(function(file) {
        ...
        if (hljs) {
            source.code = hljs.highlight(source.code, {language:'javascript'}).value;
        ...
        //ugly ordering, left-ellipsis re-orders '/' symbols so we need /module/item  and not module/item/
        let title = namespaceTextToHeading(sourceFiles[file].shortened,
            "", "text-bold left-ellipsis", '/', '/', '/', '');
        generate('Source: ' + title, [source], sourceOutfile,
            false);
    });
}
...
function linktoExternal(longName, name) {
    return linktoHierarchy(longName, name.replace(/(^"|"$)/g, ''));
}
...
function buildNav(members) {
    var nav = '<h2 class="f1-light"><a href="index.html">Home</a></h2>';
    var seen = {};
    var seenTutorials = {};
    ...
    if (members.globals.length) {
        ...
        members.globals.forEach(function(g) {
            if ( g.kind !== 'typedef' && !hasOwnProp.call(seen, g.longname) ) {
                globalNav += '<li>' + linktoHierarchy(g.longname, g.name) + '</li>';
            }
        ...
        if (!globalNav) {
            ...
            nav += '<h3>' + linktoHierarchy('global', 'Global') + '</h3>';
        }
        else {
    ...
    nav += buildMemberNav(members.modules, 'Modules', {}, linktoHierarchy);
    nav += buildMemberNav(members.namespaces, 'Namespaces', seen, linktoHierarchy);
    nav += buildMemberNav(members.externals, 'Externals', seen, linktoExternal);
    nav += buildMemberNav(members.classes, 'Classes', seen, linktoHierarchy);
    nav += buildEventsNav(members.events, seen, linktoHierarchy);
    nav += buildMemberNav(members.mixins, 'Mixins', seen, linktoHierarchy);
    nav += buildMemberNav(members.tutorials, 'Tutorials', seenTutorials, linktoTutorial);
    nav += buildMemberNav(members.interfaces, 'Interfaces', seen, linktoNice);
...
exports.publish = function(taffyData, opts, tutorials) {
    ...
    data().each(function(doclet) {
        ...
        if (doclet.examples) {
            // an `@example` tag with no body parses to an undefined entry
            doclet.examples = doclet.examples.filter(Boolean).map(function(example) {
                var caption, code;
    ...
    generate('xOpat API Documentation',
        packages.concat(
