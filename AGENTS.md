...
- `devOnly: true` gates loading against the server's dev mode (`--dev` / `XOPAT_DEV_MODE`). Unlike `stability`, it **does** refuse the element — and any plugin that requires it — outside dev mode. Declare it on anything that exists to give a developer, or a model driving one, access the application would not otherwise grant (`modules/chat-based-tester` is the reference case: it hands an LLM unrestricted page JavaScript). It is not a substitute for the element's own runtime check; it is the wall in front of it.
