...
class BVSelect {
                ...
                })
    {
        ...
        this.SelectNone = function () {
            ...
            selectedMultiple = [];
            let index = this.searchbox == true ? 0 : 1, node = selector[index];
            while (node) {
                node.selected = false;
                node = selector[++index];
            }
                    ...
                    if (item.classList.contains("bv_disabled") || item.classList.contains("nofocus") || item.classList.contains("bv_separator")) {} else {
                        if (selected_option.attributes.multiple) {
                            ...
                            if (selectedMultiple.indexOf(index) > -1) {
                                ...
                                selector[index + numberless].selected = false;
                                item.style.backgroundColor = "transparent";
                            } else {
                            ...
                            // Get Index of option
                            selector.getElementsByTagName('option')[index + numberless].selected = 'selected';
