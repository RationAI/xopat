...
(function($, Kinetic) {
    ...
    var methods = {
        init: function(options) {
            return this.each(function() {
                ...
                that.enjoyhint_svg_wrapper = $("<div>", {
                    class: that.cl.svg_wrapper + " " + that.cl.svg_transparent + " position-fixed"
                }).appendTo(that.enjoyhint);
                that.$stage_container = $(
                    '<div id="' + that.cl.kinetic_container + '" class="position-fixed">'
                ).appendTo(that.enjoyhint);
                that.$canvas = $(
                    '<canvas id="' +
                    canvas_id +
                    ...
                    that.canvas_size.h +
                    '" class="' +
                    that.cl.main_canvas + ' position-fixed">'
                ).appendTo(that.enjoyhint);
                that.$svg = $(
                    '<svg width="' +
                    that.canvas_size.w +
                ...
                var $top_dis_events = $("<div>", {
                    class: that.cl.disable_events_element + " position-fixed"
                }).appendTo(that.enjoyhint);
                    ...
                    switch(label_hor_side) {
                        ...
                        case 'oversized':
                            setTimeout(function(){
                                ...
                                $('.enjoyhint_next_btn').css(highlighted);
                                $('.enjoyhint_prev_btn').css(highlighted);
                                $('.enjoyhint_skip_btn').css(highlighted);
                            }, 450)
                            label_y = central_ver_position
                            label_x = central_position;
                            break;
        ...
        render_circle: function(values={}) {
            this.each(function() {
                this.enjoyhint_obj.renderCircle(values);
            });

            return this;
        },

        render_rect: function(values={}) {
            this.each(function() {
                this.enjoyhint_obj.renderRect(values);
            });
...
var EnjoyHint = function(configs) {
    ...
    var stepAction = function(step=undefined) {
        ...
        if (step < 0) {
            //so as not to end in endless loop, makes it go forwards
            step = 0;
            current_step = 0;
        };

        var step_data = data[step];
        ...
        if (!step_data.selector) {
            for (var prop in step_data) {
                ...
                let parsed = prop.split(" ");
                if (step_data.hasOwnProperty(prop) && parsed[1]) {
                    step_data.event = parsed[0];
                    ...
                    if (
                        ...
                        step_data.event_type = parsed[0];
                    }

                    step_data.description = step_data[prop];
                    parsed.splice(0, 1);
                    step_data.selector = parsed.join(" ");
                }
            }
        }
        ...
        if (! $(step_data.selector).length) {
            ...
            return;
        } else if (step_data.runIf && !step_data.runIf()) {
            //runIf is a predicate that enables to skip over a particular step
            stepAction(current_step > step ? step-1 : step + 1);
            return;
        }
        current_step = step;
        ...
        $enjoyhint.addClass("position-fixed");
        ...
        setTimeout(function() {
            ...
            setTimeout(function() {
                ...
                if(step_data.showPrev && current_step !== 0) {
                ...
                if (step_data.event_type) {
                    ...
                    $event_element.on(event, function(e) {
                        ...
                        stepAction(current_step+1); // clicked
                    });
                }
    ...
    that.rePaint = function() {
        if ($event_element != null) {
            let bbox = $event_element[0].getBoundingClientRect();
            $body.enjoyhint("redo_events_near_rect", bbox);

            let center_x = bbox.left + bbox.width/2;
            let center_y = bbox.top + bbox.height/2;

            let step_data = data[current_step];
            if (step_data) {
                switch (data.shape || "rect") {
                    case "circle":
                        $body.enjoyhint("render_circle", {
                        ...
                        break;
                    case "rect":
                        $body.enjoyhint("render_rect", {
                            x: center_x, y: center_y,
                            w: bbox.width+10, h: bbox.height+10
                        });
                        break;
                }
            }
