...
3. **The tour engine** (`src/classes/app/tutorial/`, reached as `APPLICATION_CONTEXT.tutorials`) — the actual step driver, with the visual styling tuned to DaisyUI (glass exit pill, readable label, gradient Next/Prev buttons).
...
- **`action`** — `next` (advance via the tour's NEXT button) or `click` (advance only when the user actually clicks the selector — useful for opening a collapsed panel as part of the walk).
- **`css-selector`** — any CSS selector (resolved with `document.querySelector`). See the cookbook below.
...
Runs once when the user clicks the card, before the tour takes over. Use it to put the UI into the known starting state your steps assume — e.g. close floating windows, scroll a list to the top. Don't put per-step set-up here; that belongs in `runIf` (or in an earlier `click` step).
...
xOpat supports multiple viewers in a single page. Per-viewer DOM elements get the viewer's position id as a prefix — e.g. `osd-0-right-menu-menu-b-opened-shaders` in a single-viewer session, `osd-0-…` and `osd-1-…` in a two-viewer session. Hard-coding `#osd-0-…` would walk the wrong viewer in some setups; the suffix selector (`[id$="-right-menu-menu-b-opened-shaders"]`) resolves through `document.querySelector` to the **first** matching element, which is the focused viewer in the layouts core ships today. (jQuery is not loaded in xOpat — `$` is the i18n namespace.)
...
### The tour can't highlight collapsed elements

Layers and Navigator tabs **start collapsed** on first load. The `next` highlight needs the target visible; if you point at something inside a closed panel, the tour will look broken.
...
- Keep step text under ~200 characters; longer text is fine when needed but wraps awkwardly in the step label.
...
5. Hit the glass X in the button row mid-tour — confirm the close cleans up without leaving residual overlays.
...
- `src/assets/custom.css` (`.xo-tour-*`) — visual styling of the in-step label and buttons. DaisyUI supplies the button shape; the palette is pinned there because the overlay is dark in every theme.
