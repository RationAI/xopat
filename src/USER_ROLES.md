...
| --------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------- | ------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `{ id: "crud:annotation", kind: "crud" }` on owner `annotations` | `annotations.crud:annotation.create`, `.read`, `.update`, `.delete`                                       | `allow` | One `pre-create` / `pre-update` / `pre-delete` guard each, registered with `priority: 10000` so the role check wins over **any other guard** — a domain guard cannot mask a denial with its own message. (Note the owner's `def.validate` still runs first of all, ahead of the whole guard phase; keep validators permissive if you care about which message a denied user sees.) `.read` is declared but has no pre-phase today. |
